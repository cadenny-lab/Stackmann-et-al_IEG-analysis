...
To start, load data from this study, located at NCBI GEO accession number [**GSE315540**](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE315540).

These data are available as 1) raw FASTQ read files, 2) barcode/feature matrices from Cell Ranger, and 3) filtered count matrices as H5 files.
...
#populate sample info, to be used for data loading (set manually)
  samples <- data.frame(file.path =   c("Sample1_filtered.h5",
                                        "Sample2_filtered.h5",
                                        "Sample3_filtered.h5",
                                        "Sample4_filtered.h5",
                                        "Sample5_filtered.h5",
                                        "Sample6_filtered.h5"),
                        sample.name = c("Sample1", "Sample2", "Sample3", "Sample4", "Sample5", "Sample6"),
...
#merge filtered Seurat objects into one combined Seurat
  HPC <- merge(x = SampleList[[2]], y = SampleList[c(1, 3:length(SampleList))], #`merge()` only has two input slots, x and y
               add.cell.ids = as.character(c(2, 1, 3:length(SampleList))), #add sample # at the start of barcode
